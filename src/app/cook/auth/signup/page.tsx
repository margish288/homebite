  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (cookData.success) {
        ...
        setError(
          "Account created but failed to set up cook profile. Please complete your profile later."
        );
        router.push("/");
      }
    } catch (err) {
          ...
          {cuisineOptions.map((cuisine) => (
            <label key={cuisine} className='flex items-center space-x-2'>
              <input
                ...
                onChange={(e) =>
                  handleArrayChange("cuisine", cuisine, e.target.checked)
                }
    ...
    <div className='space-y-4'>
          ...
          {daysOfWeek.map((day) => (
            <label key={day} className='flex items-center space-x-2'>
              <input
                ...
                onChange={(e) =>
                  handleArrayChange("availableDays", day, e.target.checked)
                }
                className='rounded border-gray-300 text-primary-400 focus:ring-primary-400'
              />
      ...
      <div className='space-y-3'>
        ...
        <label className='flex items-start space-x-3'>
          ...
          />
          <span className='text-sm text-ink-light'>
            I understand that my account will be subject to verification
            including KYC, kitchen hygiene check, and license validation before
            I can start accepting orders.
          </span>
        </label>
      </div>
    </div>
